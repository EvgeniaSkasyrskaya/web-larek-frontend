...
2. Документирование
3. Типизация (данныe API, интерфейсы моделей и представлений, описание событий брокера)

#### 2 этап
3. Реализация базового кода (View, Model, Presenter)
4. Увязка модели данных с API
...
Методы:
- toggleModal(state: boolean) - переключает активность модального окна
- handleEscape(evt: KeyboardEvent) - закрывает модальное окно по кнопке Escape
...
Класс также имеет свойства items и total, которые устанавливаются через сеттер, принимая в параметры типы HTMLElement[] и number соответственно. Кроме того, класс имеет метод toggleButton(state: boolean): void для управления статусом блокировки кнопки.

#### Класс Form
Расширяет класс Component для типа данных IFormState. Предназначен для отображения формы, содержащей поля ввода. Конструктор принимает HTMLFormElement и экземпляр класса EventEmitter для возможности инициации событий. 
- constructor(container: HTMLElement, events: IEvents)
...
#### Класс OrderForm
Расширяет класс Form для типа данных IDeliveryInfo. Предназначен для реализации формы для заполнения информации о доставке и оплате. Конструктор принимает HTMLFormElement и экземпляр класса EventEmitter для возможности инициации событий, а также устанавливает слушатели на изменения поля ввода и на сабмит формы. 
- constructor(container: HTMLElement, events: IEvents)\
...
#### Класс ContactsForm
Расширяет класс Form для типа данных IContacts. Предназначен для реализации формы для заполнения контактных данных покупателя. Конструктор принимает HTMLFormElement и экземпляр класса EventEmitter для возможности инициации событий, а также устанавливает слушатели на изменения полей ввода и на сабмит формы. 
- constructor(container: HTMLElement, events: IEvents)\
...
*Список всех событий, которые могут генерироваться в системе, типизирован в перечислении AppEvents*
